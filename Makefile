...
APP_OBJ = $(addprefix $(BUILD_DIR)/,$(notdir $(APP_SRC_C:.c=.o)))
APP_OBJ += $(addprefix $(BUILD_DIR)/,$(notdir $(APP_SRC_S:.s=.o)))
vpath %.c $(sort $(dir $(APP_SRC_C)))
vpath %.s $(sort $(dir $(APP_SRC_S)))
...
OPENOCD_INTERFACE = /usr/share/openocd/scripts/interface/ftdi/ft232h.cfg
OPENOCD_TARGET = /usr/share/openocd/scripts/target/lpc17xx.cfg

flash-app: update_bin
	$(Q)openocd -d0 -f $(OPENOCD_INTERFACE) -f $(OPENOCD_TARGET) -c "program $(BUILD_DIR)/"$(APP_ELF)" reset exit"

fw-debug: update_bin
	$(Q)openocd -d0 -f $(OPENOCD_INTERFACE) -f $(OPENOCD_TARGET) &
	$(Q)sleep 1
	$(Q)if pidof openocd 2>/dev/null; then \
        $(DB) $(BUILD_DIR)/$(APP_ELF); \
        pgrep -f openocd | xargs kill; \
    else \
        exit 1; \
    fi
